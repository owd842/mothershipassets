...
REM pipe is created by adobeupdate, prevent entry if adobeupdate is running
...
wmic process where (Name="cmd.exe") get Name,ProcessId,ParentProcessID,executablepath,CommandLine /format:csv | findstr launch.cmd > %process_check%
...
IF %LineCount% GTR 1 (
    echo launch.cmd is running -- exiting >> %logfpath%
    exit
)
