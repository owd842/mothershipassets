...
#mothership = "https://darksalmon-crow-356809.hostingersite.com"
#mothership = "https://seashell-raven-793508.hostingersite.com"
#mothership = "http://s1083932807.online-home.ca"
...
def connect_host(tclientid):
    url = f"{mothership}/ow/relay.php?action=connecthost&clientid={tclientid}"
...
relaysessionid = connect_host(clientid)

if not relaysessionid:
    logmsg(f"unable to obtain relaysessionid -- exiting")
    sys.exit(1)
...
config.uuid = f"host_31415926"

pubnub = PubNub(config)
target_channel = f"clientid_{clientid}_sessionid_{relaysessionid}_client"
receive_channel = f"clientid_{clientid}_sessionid_{relaysessionid}_host"
...
def publish_msg(msgdict):
    ...
    logmsg(f"pubnub: publishing message on {target_channel}")
    result = pubnub.publish().channel(target_channel).message(msgdict).pn_async(publish_callback)
    return result
...
def hostpushmessage(msgobj):
    ...
    #url = f"{mothership}/ow/relay.php?action=hostpushmessage&clientid={clientid}&sessionid={relaysessionid}"
